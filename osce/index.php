...
if (isset($_POST['registrationForm'])) {
    ...
    if (!($osceCheck->num_rows) > 0) {
        $osceAdd = $conn->prepare("INSERT INTO osce (studentID, uniID, osceCreated) VALUES (?, ?, ?)");
        $osceAdd->bind_param("sss", $studentID, $uniID, $date);
        $osceAdd->execute();
                                ...
                                <table class="table table-striped table-hover">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class='t-center'>Year Level</th>
                                            <th class='t-center'>Created</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php
                                            $stmt = $conn->query("SELECT * FROM osce WHERE uniID = $uniID ORDER BY osceCreated ASC");
                                            while($row = $stmt->fetch_assoc()) {
                                                $osceID = $row['osceID'];
                                                $studentID = $row['studentID'];
                                                $osceCreated = $row['osceCreated'];

                                                $studentCheck = $conn->query("SELECT * FROM students LEFT JOIN osce ON students.studentID = osce.studentID WHERE osce.studentID = $studentID");
                                                while($row = $studentCheck->fetch_assoc()) {
                                                    
                                                    $stmt = $conn->query("SELECT * FROM students WHERE studentYearLevel = '2'");
                                                    while($row = $stmt->fetch_assoc()) {
                                                        $studentID = $row['studentID'];
                                                        ...
                                                        echo "
                                                            <tr>
                                                                <td class='t-center'>$osceID</td>
                                                                <td class='t-center'>$studentFirstName $studentLastName</td>
                                                                <td class='t-center'>$studentYearLevel</td>
                                                                <td class='t-center'>" .date('Y-m-d H:i:s', strtotime($osceCreated)). "</td>
                                                            </tr>";
                                                    }
                                                }
                                            }
                                        ...
                                        <?php
                                            ...
                                            while($row = $stmt->fetch_assoc()) {
                                                ...
                                                while($row = $studentCheck->fetch_assoc()) {
                                                    ...
                                                    $stmt = $conn->query("SELECT * FROM students WHERE studentYearLevel = '3'");
                                                    while($row = $stmt->fetch_assoc()) {
                                                        $studentID = $row['studentID'];
                                                        $studentNumber = $row['studentNumber'];
                                                        $studentFirstName = $row['studentFirstName'];
